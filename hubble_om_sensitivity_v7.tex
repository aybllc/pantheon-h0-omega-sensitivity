...
\usepackage{amsmath}
\usepackage{graphicx}
% booktabs removed — MNRAS requires \hline style tables
\usepackage{url}
\usepackage[switch]{lineno}    % line numbers for editing/review prints
\usepackage{etoolbox}          % required for math-env lineno patch
\usepackage{tabularx}          % X column for full-width annotation notes table

% ---------------------------------------------------------------
% Patch every display-math environment so equations get line numbers.
% Every line has a unique number — no exceptions.
% ---------------------------------------------------------------
\newcommand*\patchAmsMathForLineno[1]{%
  ...
  \renewenvironment{#1*}%
    ...
    {\csname oldend#1*\endcsname\endlinenomath}}
\AtBeginDocument{%
  \patchAmsMathForLineno{equation}%
  \patchAmsMathForLineno{align}%
  \patchAmsMathForLineno{alignat}%
  \patchAmsMathForLineno{gather}%
  \patchAmsMathForLineno{multline}%
  \patchAmsMathForLineno{flalign}%
}

% ---------------------------------------------------------------
% Patch MNRAS SFB caption commands so caption lines get line numbers.
% MNRAS does not use \@makecaption — it uses \SFB@makefigurecaption
% and \SFB@maketablecaption. The multi-line branch outputs text
% directly; add \internallinenumbers there.
% ---------------------------------------------------------------
\makeatletter
% Patch \start@SFBbox so abstract and keywords get line numbers.
% MNRAS wraps abstract/keywords in a \vbox via \start@SFBbox;
% lineno cannot reach inside a \vbox without \internallinenumbers.
\let\orig@start@SFBbox\start@SFBbox
\def\start@SFBbox{%
  \orig@start@SFBbox
  \internallinenumbers
}
\long\def\SFB@makefigurecaption#1#2{\vskip 6pt
 ...
 \ifdim \wd\@tempboxa >\hsize
   {\internallinenumbers\reset@font\small{\bf #1}%
     \if@cont \ -- {\it continued}\else{\bf .}\fi\ {\rm #2}\par}%
 \else
   \ifst@rredfloat
...
% Match published MNRAS style: all links blue, author names not separately linked
...
% REVIEW PRINT ONLY — remove audit stamp before submission
\title[H$_0$ sensitivity to $\Omega_m$ in Pantheon+ [p6]]{%
...
\begin{keywords}
cosmological parameters -- distance scale -- supernovae: general -- dark energy -- cosmology: observations
\end{keywords}
...
coordinate $\xi \equiv d_c/d_H$ \citep{martin2026uha}, which is by construction
...
Type~Ia supernovae are standardisable candles via the luminosity--decline
relation \citep{phillips1993} and its two-parameter stretch--colour
extension \citep{tripp1998}, first used as cosmological distance
indicators by \citet{riess1998} and \citet{perlmutter1999}.
The Pantheon+ dataset \citep{scolnic2022,brout2022}, building on the
original Pantheon compilation \citep{scolnic2018}, is the most
...
We work directly with the released Pantheon+SH0ES.dat file,
...
specifically the file Pantheon+SH0ES.dat from the public
data release at github.com/PantheonPlusSH0ES/DataRelease.
This catalogue provides, for each supernova:
redshift in the CMB frame (zCMB);
the SH0ES-calibrated distance modulus (MU\_SH0ES);
its diagonal uncertainty (MU\_SH0ES\_ERR\_DIAG);
the Cepheid-based geometric distance modulus for calibrator
SNe~Ia (CEPH\_DIST); and a binary calibrator
flag (IS\_CALIBRATOR).
...
host-galaxy correlations \citep{brout2022,popovic2023,jones2022,carr2022}.
...
The diagonal uncertainty (MU\_SH0ES\_ERR\_DIAG) increases with redshift as the photometric noise floor of the high-$z$ surveys grows (Fig.~\ref{fig:sigma_vs_z}).

\begin{figure}
  \includegraphics[width=\columnwidth, alt={Scatter plot of distance-modulus uncertainty versus redshift for 1616 Hubble-flow Type Ia supernovae. Grey points show individual supernovae; orange curve shows the running mean. Uncertainty increases at high redshift, reflecting the photometric noise floor of high-z surveys.}]{sigma_vs_z}
  \caption{Diagonal distance-modulus uncertainty
    ($\sigma_\mu$) as a function of redshift
...
IS\_CALIBRATOR~$= 1$, hosted in galaxies with
...
IS\_CALIBRATOR~$= 0$ and $z_\mathrm{CMB} > 0.005$,
...
The redshift distribution of the Hubble-flow sample is shown in Fig.~\ref{fig:z_hist}.

\begin{figure}
  \includegraphics[width=\columnwidth, alt={Histogram of the redshift distribution of 1616 Hubble-flow Type Ia supernovae in log10(z_CMB) bins. Four colours indicate equal-count quartile bins. Vertical dashed lines mark quartile boundaries at z equals 0.031, 0.181, and 0.334.}]{z_distribution}
...
\citep{hogg1999} (equation~\ref{eq:Ez}).
The comoving distance to redshift $z$ is \citep{hogg1999}
\begin{equation}
  d_c(z) = \frac{c}{H_0}\int_0^z \frac{dz'}{E(z')}.
  \label{eq:dc}
\end{equation}
Equation~(\ref{eq:dc}) gives the comoving distance to redshift $z$.
...
\begin{equation}
  ...
  \label{eq:xi}
\end{equation}
where $d_H \equiv c/H_0$ is the Hubble distance \citep{hogg1999}.
The $\xi$-coordinate was introduced as a Hubble-tension diagnostic
in \citet{martin2026uha}.
...
\end{equation}
The explicit form of equation~(\ref{eq:xi_explicit}) makes clear that the $H_0$ dependence has cancelled exactly.
...
The luminosity distance is $d_L = (1+z)\,d_c$ \citep{hogg1999}, so the
theoretical distance modulus is \citep{hogg1999}
\begin{equation}
...
\textsc{astropy}'s \textsc{FlatLambdaCDM} class
...
\begin{equation}
  ...
  \label{eq:M_fixed}
\end{equation}
where $\mu_i^\mathrm{Ceph}$ is the Cepheid geometric distance modulus
and $\sigma_i$ is the diagonal distance-modulus uncertainty.
Evaluating equation~(\ref{eq:M_fixed}) yields $M_\mathrm{fixed} = +0.0802$~mag (Fig.~\ref{fig:calibrator_plot}).
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Scatter plot comparing Cepheid geometric distance modulus versus SH0ES photometric distance modulus for 77 calibrator Type Ia supernovae. Dashed line shows the 1:1 relation. The weighted mean vertical offset gives M_fixed equals +0.0802 mag. Error bars show diagonal distance-modulus uncertainty.}]{calibrator_ceph_vs_mu}
  \caption{Cepheid geometric distance modulus ($\mu_\mathrm{Ceph}$)
    vs SH0ES photometric distance modulus ($\mu_\mathrm{SH0ES}$)
    ...
    Error bars show the diagonal uncertainty $\sigma_\mu$.}
...
The signal-to-noise ratio is defined by equation~(\ref{eq:snr}).
...
$|{\Delta H_0}/{H_0}| = 5.64/70.2 = 8.4$\,\%
\citep{riess2022,planck2020}, so the
...
At low $z$, $E(z) \approx 1 + (3\Omega_m/2)z$ \citep{hogg1999}, so the $\Omega_m$
...
The per-quartile $\xi$-residual statistics are summarised in Table~\ref{tab:xi_stats} and Fig.~\ref{fig:xi_per_zbin}.

\begin{figure}
  \includegraphics[width=\columnwidth, alt={Bar chart of mean absolute fractional xi-residual in four redshift quartile bins. Bars show the Omega_m geometric sensitivity per bin; dashed orange line marks the global mean of 0.56 per cent. The residual grows monotonically from the lowest to the highest redshift bin.}]{ximid_vs_z_bins}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Scatter plot of fractional xi-residual magnitude versus redshift for all 1671 Pantheon+ Type Ia supernovae, shown as grey points with an orange running mean. Horizontal dashed line marks the global mean of 0.56 per cent. Residuals grow systematically with redshift, reflecting increasing Omega_m sensitivity of the comoving integrand at higher z.}]{xi_vs_z_scatter}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Three-panel figure comparing xi-space comoving coordinates under SH0ES and Planck frameworks. Left panel: xi versus redshift for both frameworks, nearly indistinguishable. Centre panel: absolute xi-residual on a log scale showing divergence at higher z. Right panel: histogram of log10 of absolute xi-residual; dashed red line marks the nominal delta-H0 over H0 fractional gap for reference.}]{gaia_zero_bias_plot}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Cumulative distribution function of the fractional xi-residual for all 1671 Pantheon+ Type Ia supernovae. Approximately 50 per cent of supernovae have residuals below 0.4 per cent and 90 per cent have residuals below 1.5 per cent. Vertical dashed line marks the global mean of 0.56 per cent.}]{xi_residual_cdf}
...
\end{equation}
The signal-to-noise ratio (equation~\ref{eq:snr}) is $\mathrm{S/N} = 46$.
...
nuisance treatment, or alternative likelihood choices within the adopted pipeline.

\begin{figure}
  \includegraphics[width=\columnwidth, alt={Histogram of the bootstrap distribution of delta-H0 equals H0(Omega_m=0.334) minus H0(Omega_m=0.315) over 1000 resamples of 1616 Hubble-flow Type Ia supernovae. The distribution is entirely negative, with mean minus 0.218 and standard deviation 0.005 km per second per megaparsec. Vertical line marks the mean; shading indicates the 16th to 84th percentile interval.}]{bootstrap_histogram}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Line plot of best-fit H0 in km per second per megaparsec versus assumed matter density Omega_m, from the Cepheid-anchored refit of 1616 Pantheon+ Hubble-flow supernovae. The relation is linear with slope approximately minus 11.6 km per second per megaparsec per unit Omega_m. Vertical dashed lines mark Omega_m=0.315 (Planck, orange) and 0.334 (SH0ES, blue). Shaded band shows the delta-H0=0.22 sensitivity interval.}]{refit_H0_vs_Om}
...
The $\chi^2(H_0)$ profile at the fiducial $\Omega_m = 0.334$ is a well-defined parabola (Fig.~\ref{fig:chi2_check}).
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Chi-squared profile as a function of H0 at fixed Omega_m=0.334, fitted to 1616 Hubble-flow supernovae. The profile is a well-defined parabola centred at H0=75.67 km per second per megaparsec with chi-squared minimum per degree of freedom less than 1, confirming the H0-M degeneracy is broken and diagonal uncertainties are conservatively wide.}]{chi2_sanity_check}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Overlaid chi-squared profiles as functions of H0 at Omega_m=0.334 (blue) and Omega_m=0.315 (orange). Both profiles are well-defined parabolas whose minima differ by delta-H0=minus 0.22 km per second per megaparsec, indicated by a horizontal bracket. The profiles are otherwise nearly identical, reflecting the small magnitude of the Omega_m sensitivity.}]{chi2_comparison}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Four-panel line plot of H0 versus Omega_m for each of the four redshift quartile bins, colour-coded to match the redshift histogram. The slope dH0/dOmega_m steepens from approximately minus 1.2 in the lowest bin to approximately minus 26 km per second per megaparsec per unit Omega_m in the highest bin. Vertical dashed lines mark Omega_m=0.315 and 0.334.}]{H0_scan_per_zbin}
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Bar chart of delta-H0 equals H0(Omega_m=0.334) minus H0(Omega_m=0.315) in four equal-count redshift quartile bins of approximately 404 supernovae each. All bars are negative and the magnitude grows monotonically with redshift. Orange dashed line marks the overall mean of minus 0.22 km per second per megaparsec. Even in the highest-z bin, the shift represents only 8.7 per cent of the nominal SH0ES-Planck tension.}]{zbin_deltaH0}
...
Our direct-refit analysis uses only the diagonal distance-modulus
uncertainties $\sigma_\mu$ of the Pantheon+ uncertainty matrix
...
Residuals of the best-fit distance moduli show no systematic redshift trend (Fig.~\ref{fig:residual_vs_z}), confirming adequate fit quality across the full redshift range.

\begin{figure}
  \includegraphics[width=\columnwidth, alt={Scatter plot of Hubble diagram residuals (observed minus theoretical distance modulus) versus redshift for 1616 Hubble-flow Type Ia supernovae at the best-fit H0 for Omega_m=0.334. Grey points show individual supernovae; orange curve shows the running mean in log-z bins. No systematic redshift trend is visible, confirming adequate fit quality across the full redshift range.}]{residual_vs_z}
...
distance modulus \citep{hogg1999},
\begin{equation}
  \mu^\mathrm{th}(z;\,H_0,\Omega_m)
  \approx 5\log_{10}\!\left[\frac{cz(1+z/2)}{H_0}\right] + 25,
  \label{eq:mu_lowz}
\end{equation}
(equation~\ref{eq:mu_lowz}) is sensitive to $\Omega_m$ only through terms of order
...
\end{equation}
Equation~(\ref{eq:sigma_omegam}) gives $\sigma_{\Delta H_0}^{\Omega_m} \approx 0.22$~km\,s$^{-1}$Mpc$^{-1}$, which broadens the allowed range of the $\Omega_m$-driven contribution
...
\citep{ivezic2019} and the Roman Space Telescope \citep{spergel2015}
will both expand the high-$z$ SN sample substantially, making this
calibration relevant for those analyses.
...
\begin{figure}
  \includegraphics[width=\columnwidth, alt={Ladder diagram comparing published H0 measurements in km per second per megaparsec from seven independent methods: Planck CMB, SH0ES distance ladder, CCHP TRGB, H0LiCOW time delays, TDCOSMO, Megamasers, and surface brightness fluctuations. Horizontal bars show 1-sigma intervals, ordered by probe type. Vertical blue band marks this work's best-fit of 75.67 at Omega_m=0.334; orange band marks 75.89 at Omega_m=0.315.}]{H0_comparison_ladder}
...
core \textsc{Python} package for Astronomy \citep{astropy2013,astropy2018,astropy2022};
...
% Annotation notes sheet is a separate file:
% journal-gates/templates/annotation_notes.tex (.pdf)
% Print alongside every preprint hard copy — never submit to journal.
