...
\begin{figure}
  ...
  \caption{Diagonal distance-modulus uncertainty
    (MU\_SH0ES\_ERR\_DIAG) as a function of redshift
...
\begin{equation}
  ...
  \label{eq:Ez}
\end{equation}
where $\Omega_\Lambda = 1 - \Omega_m$ for a spatially flat universe (equation~\ref{eq:Ez}).
...
\begin{equation}
  ...
  \label{eq:M_fixed}
\end{equation}
where $\mu_i^\mathrm{Ceph} = CEPH\_DIST_i$ is the Cepheid
geometric distance modulus and $\sigma_i = MU\_SH0ES\_ERR\_DIAG_i$.
...
\begin{figure}
  ...
  \caption{Cepheid geometric distance modulus (CEPH\_DIST)
    vs SH0ES photometric distance modulus (MU\_SH0ES)
    ...
    Error bars show the diagonal uncertainty MU\_SH0ES\_ERR\_DIAG.}
...
MU\_SH0ES\_ERR\_DIAG of the Pantheon+ uncertainty matrix
...
% ---------------------------------------------------------------
% Trailing annotation notes page — editing/review use only
% Remove or comment this block before journal submission
% ---------------------------------------------------------------
\clearpage
\nolinenumbers
\onecolumn
...
\thispagestyle{empty}
\begin{center}
  {\large\bfseries Annotation Notes}\\[4pt]
  {\small\ttfamily hubble\_om\_sensitivity\_v7.tex \quad\textbar\quad Printed \today}
\end{center}
\vspace{6pt}
...
\hline
\multicolumn{1}{|c|}{\textbf{Line \#}} & \textbf{Note} \\
\hline
 & \\ \hline
 & \\ \hline
 & \\ \hline
