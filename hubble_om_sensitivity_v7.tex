...
Our measurements provide precisely this verification.
  ...
  \begin{tabular}{ccrcccr}
    ...
    Bin & $z$ range & $n$
        ...
        & & & \multicolumn{2}{c}{(km\,s$^{-1}$\,Mpc$^{-1}$)}
              ...
              & (km\,s$^{-1}$\,Mpc$^{-1}$\,unit$^{-1}$)\\
    \midrule
    1 & $0.005$--$0.031$ & 403 & 75.71 & 75.74 & $-0.023$ & $-1.2$ \\
    2 & $0.031$--$0.181$ & 405 & 75.61 & 75.71 & $-0.094$ & $-4.9$ \\
    3 & $0.181$--$0.334$ & 404 & 75.79 & 76.06 & $-0.265$ & $-14.0$ \\
    4 & $0.334$--$2.261$ & 404 & 75.42 & 75.91 & $-0.494$ & $-26.0$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{equation}
  \sigma_{\Delta H_0}^{\Omega_m}
    ...
    \approx 0.24\;\text{km\,s}^{-1}\text{Mpc}^{-1}.
...
Quantitatively, the $\xi$-space result of 0.56\,\% translates
into a predicted $\Delta H_0$ as follows.
The mean $\langle\xi\rangle = 0.197$ corresponds to a mean
comoving distance of $d_c = 0.197 d_H = 0.197 c/H_0$.
A 0.56\,\% shift in $\xi$ at fixed $H_0$ corresponds to a
0.56\,\% shift in $d_c$, which propagates to a 0.56\,\% shift
in $\mu = 5\log_{10} d_L + \mathrm{const}$ via the
$\log_{10}$ at the sample mean distance.
Through equation~(\ref{eq:chi2}), this translates to a
$\Delta H_0 / H_0 \approx 0.56$\,\%/2 $\approx 0.28$\,\%
(the factor of 2 from the $5\log_{10}$ derivative), giving
$\Delta H_0 \approx 0.28$\,\% $\times$ 75.7 $\approx 0.21$
km\,s$^{-1}$\,Mpc$^{-1}$.
This is consistent with the bootstrap measurement of
$0.218 \pm 0.005$ km\,s$^{-1}$\,Mpc$^{-1}$, confirming that
both diagnostics are measuring the same quantity.
...
sensitivity (Table~\ref{tab:zbins}) implies that future
supernova surveys at $z > 0.3$ will be more sensitive to the
$\Omega_m$ prior than current datasets dominated by low-$z$ SNe.
The Vera C. Rubin Observatory Legacy Survey of Space and Time
and the Roman Space Telescope will both dramatically expand the
high-$z$ SN sample, making this calibration increasingly important.
...
\citep{riess2023,breuval2024,anderson2024}.
...
\item The two diagnostics are mutually consistent, with the
  $\xi$-space result predicting $\Delta H_0 \approx 0.21$
  km\,s$^{-1}$\,Mpc$^{-1}$ from geometric considerations alone,
  in excellent agreement with the bootstrap measurement.
  This cross-validation confirms the internal consistency of
  the pipeline.
...
\begin{figure}
  ...
  \caption{$\xi$-space comparison of the SH0ES and Planck frameworks
    for all 1{,}671 SNe~Ia as a function of redshift.
    Upper panel: $\xi_\mathrm{SH0ES}(z)$ (blue) and
    $\xi_\mathrm{Planck}(z)$ (orange) vs $z_\mathrm{CMB}$.
    The two curves are nearly indistinguishable on this scale.
    Lower panel: fractional residual
    $\Delta_\xi = |\xi_\mathrm{SH0ES} - \xi_\mathrm{Planck}|/\xi_\mathrm{mid}$
    vs $z$, showing the $\Omega_m$-driven divergence at higher $z$.}
...
\begin{figure}
  ...
  \caption{$\xi$-space residual statistics per redshift quartile,
    ...
    Bars show $\langle|\Delta\xi|\rangle/\langle\xi\rangle$
